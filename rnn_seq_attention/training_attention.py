...
import pandas as pd
from lstm import LSTMDecoderOnly
...
def train_step(
    ...
    logits, states = model(input, states) # logits shape = [B, L, VOCAB_SIZE]
    ...
    loss: torch.Tensor = loss / config['accum_steps']
...
def eval_step(
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, total=len(dataloader), desc='Evaluating'):
            ...
            logits_tf, states = model(input, None)
